...
| Doc | Scope |
|---|---|
| [`team-08-infrastructure-platform.md`](./team-08-infrastructure-platform.md) | Azure cloud architecture (D03), compute (Container Apps/AKS-GPU/serverless), storage, **event backbone (D08 final word)**, **databases + caching (D09 final word)**, CDN/edge, monitoring/observability + SLOs, cost optimization + cost model at all three D02 scale points, global-scale multi-region + data residency, disaster recovery (RPO/RTO), infrastructure diagrams. |
| [`team-09-security-privacy-compliance.md`](./team-09-security-privacy-compliance.md) | Security architecture, **STRIDE threat model**, consent management (two-party, per-session — D10), audio privacy + no-retention mode, encryption (TLS/AES/Key Vault/tenant isolation), retention + deletion, GDPR/CCPA, HIPAA, enterprise controls (SSO/SAML/SCIM/RBAC/IP-allowlist/audit), audit trails, compliance roadmap (SOC 2 Type II → HIPAA → GDPR/ISO). |
...
```
   ...
   ───────────────────────────────────────     ──────────────────────────────────
   Private VNet / NSGs / Private Link       →   zero-trust network (no public data)
   Key Vault keys (per-tenant, per-region)  →   encryption + crypto tenant isolation
   RLS-capable PostgreSQL Flexible Server + scoped Redis/Blob → tenant data isolation (T6)
   Activity logs / Cosmos DB append / Blob WORM → immutable audit + consent ledger
   ...
   Silo option (dedicated cluster + key)    →   HIPAA / enterprise isolation
```
...
- **D08 (event backbone) — CONFIRMED + refined:** **Kafka-compatible log (Event Hubs
  Dedicated)** at scale, **Event Hubs** at MVP, both behind a thin **EventBus**
  abstraction; Azure Service Bus/Azure Event Grid/Azure Durable Functions for control + async.
  Other lanes assume a **durable, ordered, replayable, per-`session_id` stream**
  addressed by logical topic name.
- **D09 (datastores) — CONFIRMED + refined:** Azure Database for PostgreSQL Flexible Server (+pgvector at MVP)
  → dedicated vector (Azure AI Search/Pinecone) and managed graph DB (Cosmos DB Gremlin/Neo4j-class) at scale; Redis;
  Blob Storage; **plus Cosmos DB** for idempotency/dedup/audit-append (additive, not a
  replacement).
